 ...
 */
export function materialCurves(surface: Surface, sample: string) {
  ...
  return rgb.map(channel => Array.from({ length: 33 }, (_, index) => {
    ...
    const start = metal ? .32 : .55;
    const reflection = Math.pow(Math.max(0, (light - start) / (1 - start)), metal ? 1.65 : 1.25);
